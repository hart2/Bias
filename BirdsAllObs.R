...
Aves_EEZ <- read_csv("~/github/Aves_EEZ.csv")
bird<- Aves_EEZ %>% 
  ...
  filter(date_year >= 1940 & date_year < 2021)        #filtering for only human observations and data collection pbird 1960
...
v2 <- 1:nrow(gensp)              # num of cells in scientificName and creating vector with the number of cells necessary for running splitstring fbirdn
species <- data.frame(v1,v2)
colnames(species) <- c("scientificName", "v2")

alpha <- function(species){        # Fbirdn to filter dataframe to include only rows with a space between two charbirder strings (aka genus and species)
  booleans <- vector()
  i <- 1
...
df1 <- c(species,bird)              # Inputting "species" T/F into "div" dataframe
...
bird_nd <- invisible(unique(bird) %>%         # "invisible()" suppresses output
                      arrange('scientificName'))
rm(bird,num,v1,species,num_gs,gensp,freq,freq1,df1,df2)

# Important!!!
# bird_nd (data without duplicates),birdSpecies (frequency of appearance in 
...
bufferX <- bird_nd %>%
  filter(str_detect(scientificName, "^bird"))
...
rm(bufferA,bufferB, bufferC, bufferD, bufferE, bufferF, bufferG, bufferH, bufferI,
   ...
   bufferR, bufferS, bufferT, bufferU, bufferV, bufferW, bufferbird, bufferY, bufferZ)

# remove land_buffer from bird_nd (there are additional 300 records removed 
# bc they had duplicate catalogNumbers - which isn't supposed to happen, human error putting info into OBIS)
bird <- anti_join(bird_nd,bird_landbuffer, by = "catalogNumber") 
bird <- bird %>% 
  arrange(scientificName)

rm(bird_nd,bird_landbuffer,land_buffer)
