...
# This code will compare occurrence records of crustaceans for North and South America
...
df1 <- c(species,crust)              # Inputting "species" T/F into "div" dataframe
...
crust_nd <- invisible(unique(crust) %>%         # "invisible()" suppresses output
                      arrange('scientificName'))
rm(crust,num,v1,species,num_gs,gensp,freq,freq1,df1,df2)

# Important!!!
# crust_nd (data without duplicates),crustSpecies (frequency of appearance in 
...
# remove land_buffer from crust_nd (there are additional 300 records removed 
# bc they had duplicate catalogNumbers - which isn't supposed to happen, human error putting info into OBIS)
Crust <- anti_join(crust_nd,crust_landbuffer, by = "catalogNumber") 
Crustacean <- Crust %>% 
  arrange(scientificName)
